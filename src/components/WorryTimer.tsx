 ...
 *
 * 피그마:
 *  - timer (289:1069) — w=325, h=37, status=idle/selected 두 variants (진행 중)
 *  - timer (679:908) — "걱정타임 시간 끝난 후" — 전체 yellow + 텍스트 mainGreen, 항상 표시
 *
 * 사양:
...
// stroke 색상을 동적으로 주입 가능하도록 함수화
function clockIconXml(strokeColor: string): string {
  return `<svg viewBox="122 11 18 18" xmlns="http://www.w3.org/2000/svg" fill="none">
<path d="M124.25 20.5625C124.25 21.4489 124.425 22.3267 124.764 23.1456C125.103 23.9646 125.6 24.7087 126.227 25.3355C126.854 25.9623 127.598 26.4595 128.417 26.7987C129.236 27.1379 130.114 27.3125 131 27.3125C131.886 27.3125 132.764 27.1379 133.583 26.7987C134.402 26.4595 135.146 25.9623 135.773 25.3355C136.4 24.7087 136.897 23.9646 137.236 23.1456C137.575 22.3267 137.75 21.4489 137.75 20.5625C137.75 19.6761 137.575 18.7983 137.236 17.9794C136.897 17.1604 136.4 16.4163 135.773 15.7895C135.146 15.1627 134.402 14.6655 133.583 14.3263C132.764 13.9871 131.886 13.8125 131 13.8125C130.114 13.8125 129.236 13.9871 128.417 14.3263C127.598 14.6655 126.854 15.1627 126.227 15.7895C125.6 16.4163 125.103 17.1604 124.764 17.9794C124.425 18.7983 124.25 19.6761 124.25 20.5625Z" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M126.5 28.4376L127.54 26.3594" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M131 20.5625H128.457" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M131 16.625V20.5625" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M123.875 14.75L126.688 12.5" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M135.5 28.4374L134.46 26.3584" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M137.938 14.75L135.125 12.5" stroke="${strokeColor}" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>`;
}
...
export function WorryTimer({ elapsedSec, totalSec, selected = false, style }: WorryTimerProps) {
  ...
  // figma 679:908 — 시간 끝난 후 (timer ended): 텍스트 항상 표시 + mainGreen + 20px
  const isEnded = totalSec > 0 && elapsedSec >= totalSec;
  const showTime = isEnded || selected;
  const strokeColor = isEnded ? Colors.mainGreen : '#000000';
    ...
    <View style={[styles.frame, style]}>
      ...
      {showTime && (
        <View style={styles.timeOverlay} pointerEvents="none">
          ...
          <Text
            style={[
              isEnded ? styles.timeTextEnded : styles.timeText,
              { color: textColor },
            ]}
